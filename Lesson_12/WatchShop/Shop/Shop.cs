...
namespace WatchShop
{
    [Serializable] public class Shop
    {
        ...
        private SortEventHandler _orderBy;  // Делегат для сортировки ассортимента магазина
        ...
        private void AgentListener(object sender, ExchangeEventArgs args)   // Метод, прослущивающий агента для обмена
        {   
            if(args.Buyer == this)
                Buy(args);
        ...
        private void Buy(ExchangeEventArgs args)    // Метод, если магазин является покупателем
        {
            Money -= args.TotalCost.Value;
            args.Seller.AddMoney(args.TotalCost.Value);
        }

        private void Sell(ExchangeEventArgs args)   // Метод, если магазин является продавцом
        {
